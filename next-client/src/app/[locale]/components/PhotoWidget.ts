...
type SocialSymbolProps = {
  onClick: () => void;
}
const SocialSymbol = ({ ...props }: SocialSymbolProps): React.PureComponent => (
  <button type="button" className={`btn social-login-btn ${props.spanParent ? 'span-parent' : null} ${socialFonts[props.site + '-btn-textless']}`}>
